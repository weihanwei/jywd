 ...
 * 
 */
public class JedisObjectUtil<T>{
	 ...
	 *
	 */
	public  int saveForString(String key,String value)  throws CacheException{
		CacheException.checkCacheKey(key);
		Jedis jedis = null;
			jedis = getConnection();
			if(jedis == null)
	 ...
	 *
	 */
	public  int saveForString(String key,String value,int second)  throws CacheException{
		CacheException.checkCacheKey(key);
		Jedis jedis = null;
			jedis = getConnection();
			if(jedis == null)
	...
	public  int saveForByte(byte [] key,byte [] value,int second)  throws CacheException{
		CacheException.checkCacheKey(key);
		Jedis jedis = null;
			jedis = getConnection();
			if(jedis == null)
	 ...
	 *
	 */
	public  String queryForString(String key)  throws CacheException{
		CacheException.checkCacheKey(key);
		Jedis jedis = null;
			jedis = getConnection();
			if(jedis == null)
	...
	public String saveForBean(String key, T bean){
		String value = beanToGson(bean);
		Jedis jedis = null;
		jedis = getConnection();
		String rst = jedis.set(key, value);
		closeConnection(jedis);
		return rst;
	}
	
	public T queryForBean(String key, Class<T> beanClass){
		Jedis jedis = null;
		jedis = getConnection();
		String value = jedis.get(key);
		if(key.toLowerCase().contains("finishreceive"))
			System.out.println("JedisObjectUtil->queryForBean finishreceive value:"+value);
		T rst = gsonToBean(value, beanClass);
		closeConnection(jedis);
		return rst;
	}
	
	public void saveForBean(String key, T bean, int second){
		String value = beanToGson(bean);
		Jedis jedis = null;
		jedis = getConnection();
		Transaction transaction = jedis.multi();
	...
	private T gsonToBean(String value, Class<T> beanClass){
		Gson gson = new Gson();
		return gson.fromJson(value, beanClass);
	}
	
	private String beanToGson(T bean){
		Gson gson = new Gson();
